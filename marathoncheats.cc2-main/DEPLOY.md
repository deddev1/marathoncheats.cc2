# Deploy marathoncheats.cc

Step-by-step guide to deploy the Marathon Cheats static site to **marathoncheats.cc** on Cloudflare Pages, configure DNS, and submit to Google Search Console.

## Prerequisites

- Node.js **≥ 22.12.0**
- Cloudflare account with access to **marathoncheats.cc** DNS
...
This runs `wrangler pages deploy dist --project-name=marathoncheats2` (see `wrangler.toml`).

## 3. Custom domain and DNS

Add **marathoncheats.cc** as the primary custom domain on the Pages project.

### Apex (marathoncheats.cc)
...
2. In **Rules** → **Redirect Rules** (or Bulk Redirects), create:
   - **Source:** `www.marathoncheats.cc/*`
   - **Target:** `https://marathoncheats.cc/${1}`
   - **Status:** 301

The deployed `functions/_middleware.js` also enforces apex canonical host, legacy domain redirects (`marathoncheats.cc`, `.net`, `.com`), and legacy path redirects.
...
- `https://marathoncheats.cc/es/`
- `https://marathoncheats.cc/marathon-cheats/`
- `https://marathoncheats.cc/marathon-aimbot/`
- `https://marathoncheats.cc/sitemap.xml`
- `https://marathoncheats.cc/robots.txt`

Verify redirects:

- `http://marathoncheats.cc` → `https://marathoncheats.cc` (301)
- `https://www.marathoncheats.cc` → `https://marathoncheats.cc` (301)
- Legacy domains (e.g. `marathoncheats.cc`) → `https://marathoncheats.cc` (301)
...
2. **Add property** → choose **Domain** → enter `marathoncheats.cc`.
...
4. After verification, open **Sitemaps** and submit:
   ...
   https://marathoncheats.cc/sitemap.xml
   ```
   Remove any legacy submissions (`sitemap-index.xml`, old `marathoncheats.cc` URLs).
...
- [ ] Custom domain `marathoncheats.cc` attached and active
- [ ] `www` redirects to apex
- [ ] Legacy domains 301 to `marathoncheats.cc`
- [ ] Always Use HTTPS enabled
- [ ] `robots.txt` and sitemaps serve from `https://marathoncheats.cc`
